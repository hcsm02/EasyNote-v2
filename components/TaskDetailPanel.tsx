...
const TaskDetailPanel: React.FC<TaskDetailPanelProps> = ({ task, onClose, onUpdate }) => {
  ...
  const transcribeAudio = async (blob: Blob) => {
    ...
    try {
      ...
      setIsEditingDetails(true); // Switch to edit mode when voice input comes in
    } catch (e) {
  ...
  const handleAIFormat = async () => {
    ...
    try {
      ...
      const response = await ai.models.generateContent({
        model: 'gemini-3-flash-preview',
        contents: `Please beautify and structure the following notes. 
        CRITICAL RULES:
        1. PERSIST LANGUAGE: If the input is Chinese, output MUST be Chinese. If English, output MUST be English. NEVER translate.
        2. STRUCTURE: Use standard Markdown (bold, lists, headers) to make it highly readable.
        3. CONCISE: Keep it professional and clear.
        
        Notes to beautify: "${details}"`,
        config: {
          systemInstruction: "You are a professional document formatter. Your goal is to improve structure and readability while strictly maintaining the original language of the source text."
        }
      });
      if (response.text) {
        setDetails(response.text);
        setIsEditingDetails(false); // Switch to view mode to see formatted result
      }
    } catch (e) {
  ...
  const sendChatMessage = async (text?: string) => {
    ...
    try {
      ...
      const chat = ai.chats.create({
        model: 'gemini-3-flash-preview',
        config: {
          systemInstruction: `You are a helpful task assistant for a person working on: "${title}". 
          The task details are currently: "${details}".
          Provide concise, actionable advice or information. 
          Keep responses minimalist and polite. 
      ...
      lastResp = response.text || "I'm sorry, I couldn't process that.";
      
      setChatMessages([...newMessages, { role: 'model', text: lastResp }]);
    } catch (e) {
  ...
  return (
    <div className="fixed inset-0 z-[120] bg-[#F2F0E6] animate-in slide-in-from-right duration-500 flex flex-col overflow-hidden">
      {/* Header */}
            ...
            {!isChatOpen && (
              <div className="absolute bottom-6 right-6 flex items-center gap-3 z-10">
                ...
                <button 
                   ...
                   className={`w-14 h-14 nm-raised rounded-full flex items-center justify-center transition-all ${
                     isRecording ? 'text-red-500 nm-inset' : 'text-indigo-400'
                   }`}
                >
                   {isRecording ? (
              ...
              <div className="flex-1 overflow-y-auto p-4 space-y-4 no-scrollbar">
                ...
                {chatMessages.map((msg, idx) => (
                  <div key={idx} className={`flex flex-col ${msg.role === 'user' ? 'items-end' : 'items-start'}`}>
                    <div className={`max-w-[85%] px-4 py-2.5 rounded-[18px] text-xs font-medium leading-relaxed relative ${
                      msg.role === 'user' 
