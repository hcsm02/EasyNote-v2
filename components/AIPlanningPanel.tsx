...
const AIPlanningPanel: React.FC<AIPlanningPanelProps> = ({ onClose, onAddTasks }) => {
  ...
  const processInput = async (audioBlob?: Blob) => {
    ...
    try {
      ...
      let contents: any = `Today is ${todayStr} (${todayISO}). Request: "${input}"`;
      if (audioBlob) {
        const base64 = await blobToBase64(audioBlob);
        contents = {
          parts: [
            { inlineData: { data: base64, mimeType: 'audio/webm' } },
            { text: `Today is ${todayStr} (${todayISO}). Transcribe and create a plan with specific dates.` }
          ]
        };
      }
      ...
      const response = await ai.models.generateContent({
        ...
        contents,
        config: {
          systemInstruction: `Life Planner AI:
          1. Detect sentiment & analysis.
          2. Split into tasks. Identify 'dueDate' (YYYY-MM-DD) for each.
          3. Categories: 
             - 'history': dueDate before today
             - 'today': dueDate is today
             - 'future2': dueDate is tomorrow or next day
          ...
          responseSchema: {
            ...
            required: ["sentiment", "analysis", "tasks"]
          }
        }
      });
                    ...
                    <div className="flex-1 flex flex-col">
                      ...
                      <div className="flex gap-2 mt-1">
                         ...
                         <span className={`text-[8px] font-bold px-1.5 py-0.5 rounded-md uppercase tracking-tighter ${
                           task.isArchived ? 'bg-teal-100 text-teal-600' : 'bg-gray-100 text-gray-400'
                         }`}>
