...
const TaskItem: React.FC<TaskItemProps> = ({ task, onToggle, onEdit }) => {
  ...
  return (
    ...
    >
      ...
      >
        <div className={`w-7 h-7 rounded-xl transition-all duration-200 flex items-center justify-center ${task.selected
            ? 'nm-inset-sm bg-white/40'
            : 'nm-raised-sm'
          }`}>
          <svg
            className={`w-4 h-4 transition-colors duration-300 ${task.selected ? 'text-green-500' : 'text-gray-300'
              }`}
            fill="currentColor"
            viewBox="0 0 20 20"
          >
      ...
      <div className="flex-grow flex flex-col min-w-0">
        <span className={`text-sm font-semibold transition-all duration-300 leading-tight tracking-tight truncate ${task.selected ? 'text-gray-400' : 'text-gray-600'
          }`}>
          {task.text}
        </span>
